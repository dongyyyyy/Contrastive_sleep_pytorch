...
def check_label_info_withPath_withoutOSA( file_list,class_mode='5class',check_file='.npy'):
    if class_mode =='5class':
        labels = np.zeros(5)
    else:
        labels = np.zeros(3)
...
def stft(signal, sample_rate=200, frame_size=2, frame_stride=1, winfunc=np.hamming, NFFT=256):
    '''
