    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_dataloader,desc='Train',unit='batch') as tepoch:
            for index,(batch_signal,batch_label) in enumerate(tepoch):
                ...
                # print(f'batch_signal shape = {batch_signal.shape}')
                # print(np.bincount(batch_label))
                # exit(1)
                ...
                # print(f'pred = {pred[:10]} // batch_label = {batch_label[:10]}')
        ...
        with tqdm(test_dataloader,desc='Test',unit='batch') as tepoch:
            for index,(batch_signal, batch_label) in enumerate(tepoch):     
                ...
                with torch.no_grad():
                    ...
                    # test_total_data += len(batch_signal_200hz)
...
def training_ResNet_contrastiveLearning(use_channel=[0,1],classification_mode='5class',use_dataset='SeouUniv',gpu_num=[0]):
    ...
    model_save_path = f'/mnt/hdd3/Contrastive_Sleep/saved_model/11_01/resnet18_8_1_1_imageFormat/'
    logging_save_path = f'/mnt/hdd3/Contrastive_Sleep/log/11_01/resnet18_8_1_1_imageFormat/'
